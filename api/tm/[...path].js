module.exports = async (req, res) => {
  try {
    ...
    const tmPath = Array.isArray(segs)
      ? `/${segs.join("/")}`
      : `/${segs || ""}`;
    ...
    for (const [k, v] of Object.entries(req.query)) {
      ...
      if (Array.isArray(v)) {
        ...
        tmUrl.searchParams.append(k, String(v));
      }
    }

    // inject secret TM key from Vercel env
    ...
    res
      .status(500)
      .json({ error: "proxy_failed", message: String(err?.message || err) });
  }
};